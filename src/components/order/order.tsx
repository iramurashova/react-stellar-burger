...
const Order: FC<TOrderProps> = ({ orderInfo, isStatus }) => {
  ...
  const returnIngredients = () => {
    const newIngredients = Array.from(new Set(orderInfo.ingredients)).slice(
      0,
      6
    );
  ...
  const returnIngredientsPrice = () => {
    ...
    return arrayOfPrices.reduce((sum = 0, item) => {
      if (item) {
        sum += item;
      }
      return sum;
  ...
  return (
    <li className={`${styles.card} mb-6 pl-6 pr-6 pt-6 pb-6 mr-4`}>
      ...
      {isStatus && (
        <div
          className={`${styles.row} mt-2 text_type_main-small ${
            orderInfo.status === "done" ? "text_color_success" : ""
          }`}
        >
      ...
      <div className={`${styles.row} mt-6 mr-6`}>
        <ul className={styles.images_row}>
          {orderIngredients.map((ingredient, index) => (
            <li key={index} className={styles[`ingredient_${index}`]}>
              <img
